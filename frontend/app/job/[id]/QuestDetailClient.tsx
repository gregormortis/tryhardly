...
export default function QuestDetailClient({
  ...
  // Server-rendered quest for SEO (generateMetadata + JobPosting JSON-LD in
  // the server page). When present and matching the route id, the client skips
  // its first fetch so the page never flashes a loading skeleton.
  initialQuest?: Quest | null;
}) {
  const params = useParams();
  const router = useRouter();
  const { user, loading: authLoading } = useAuth();
  const [quest, setQuest] = useState<Quest | null>(initialQuest ?? null);
  ...
  // Tracks whether the server-provided initial quest was already consumed, so a
  // later auth resolution still refreshes the data instead of skipping forever.
  const consumedInitialQuest = useRef(false);

  useEffect(() => {
    if (
      initialQuest &&
      initialQuest.id === params.id &&
      !consumedInitialQuest.current
    ) {
      consumedInitialQuest.current = true;
      setLoading(false);
      // Owner-only applications still need the authed client call.
      if (user && initialQuest.questGiverId === user.id) {
        api
          .get<Application[]>(`/quests/${params.id}/applications`)
